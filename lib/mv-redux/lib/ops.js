...
export function equal(u, v) {
    if (isVector(u) && isVector(v) && u.length === v.length) {
        ...
        throw new Error("Invalid arguments passed to 'equal':\n" +
...
export function add(u, v) {
    if (isVector(u) && isVector(v)) {
        ...
        if (u.type === 'mat4' && v.type === 'mat4') {
            return mat4(u[0][0] + v[0][0], u[0][1] + v[0][1], u[0][2] + v[0][2], u[0][3] + v[0][3], u[1][0] + v[1][0], u[1][1] + v[1][1], u[1][2] + v[1][2], u[1][3] + v[1][3], u[2][0] + v[2][0], u[2][1] + v[2][1], u[2][2] + v[2][2], u[2][3] + v[2][3], u[3][0] + v[3][0], u[3][1] + v[3][1], u[3][2] + v[3][2], u[3][3] + v[3][3]);
        }
        else if (u.type === 'mat3' && v.type === 'mat3') {
            return mat3(u[0][0] + v[0][0], u[0][1] + v[0][1], u[0][2] + v[0][2], u[1][0] + v[1][0], u[1][1] + v[1][1], u[1][2] + v[1][2], u[2][0] + v[2][0], u[2][1] + v[2][1], u[2][2] + v[2][2]);
        }
        else if (u.type === 'mat2' && v.type === 'mat2') {
            return mat2(u[0][0] + v[0][0], u[0][1] + v[0][1], u[1][0] + v[1][0], u[1][1] + v[1][1]);
        }
    }
    const uType = u?.type ?? typeof u;
...
export function mult(u, v) {
    ...
    if ((uMat = ((isMatrix(u) || isVector(u)) && typeof v === 'number')) ||
        ...
        switch (mat.type) {
            ...
            case 'mat4': return mat4(mat[0][0] * num, mat[0][1] * num, mat[0][2] * num, mat[0][3] * num, mat[1][0] * num, mat[1][1] * num, mat[1][2] * num, mat[1][3] * num, mat[2][0] * num, mat[2][1] * num, mat[2][2] * num, mat[2][3] * num, mat[3][0] * num, mat[3][1] * num, mat[3][2] * num, mat[3][3] * num);
            case 'mat3': return mat3(mat[0][0] * num, mat[0][1] * num, mat[0][2] * num, mat[1][0] * num, mat[1][1] * num, mat[1][2] * num, mat[2][0] * num, mat[2][1] * num, mat[2][2] * num);
            case 'mat2': return mat2(mat[0][0] * num, mat[0][1] * num, mat[1][0] * num, mat[1][1] * num);
        }
    }
    else if (isMatrix(u) && isVector(v)) {
        if (u.type === 'mat4' && v.type === 'vec4') {
            return vec4((u[0][0] * v[0]) + (u[0][1] * v[1]) + (u[0][2] * v[2]) + (u[0][3] * v[3]), (u[1][0] * v[0]) + (u[1][1] * v[1]) + (u[1][2] * v[2]) + (u[1][3] * v[3]), (u[2][0] * v[0]) + (u[2][1] * v[1]) + (u[2][2] * v[2]) + (u[2][3] * v[3]), (u[3][0] * v[0]) + (u[3][1] * v[1]) + (u[3][2] * v[2]) + (u[3][3] * v[3]));
        }
        else if (u.type === 'mat3' && v.type === 'vec3') {
            return vec3((u[0][0] * v[0]) + (u[0][1] * v[1]) + (u[0][2] * v[2]), (u[1][0] * v[0]) + (u[1][1] * v[1]) + (u[1][2] * v[2]), (u[2][0] * v[0]) + (u[2][1] * v[1]) + (u[2][2] * v[2]));
        }
    ...
    if (isVector(v)) {
        ...
        switch (v.type) {
            case 'vec4':
                ...
                return ((m == 0) ? vec4(0) : vec4(v[0] / m, v[1] / m, v[2] / m, v[3] / m));
            case 'vec3':
                m = v[0] * v[0] + v[1] * v[1] + v[2] * v[2];
                return ((m == 0) ? vec3(0) : vec3(v[0] / m, v[1] / m, v[2] / m));
            case 'vec2':
                m = v[0] * v[0] + v[1] * v[1];
                return ((m == 0) ? vec2(0) : vec2(v[0] / m, v[1] / m));
        }
    }
    const vType = v?.type ?? typeof v;
...
export function negate(v) {
    if (isVector(v)) {
        switch (v.type) {
            case 'vec4': return vec4(-v[0], -v[1], -v[2], -v[3]);
            case 'vec3': return vec3(-v[0], -v[1], -v[2]);
            case 'vec2': return vec2(-v[0], -v[1]);
        }
    }
    const vType = v?.type ?? typeof v;
...
export function mix(u, v, s) {
    ...
    if (typeof u === 'number' && typeof v === 'number') {
        ...
        if (u.type == 'vec2' && v.type == 'vec2')
            return vec2(r * u[0] + s * v[0], r * u[1] + s * v[1]);
        else if (u.type == 'vec3' && v.type == 'vec3')
            return vec3(r * u[0] + s * v[0], r * u[1] + s * v[1], r * u[2] + s * v[2]);
        else if (u.type == 'vec4' && v.type == 'vec4')
            return vec4(r * u[0] + s * v[0], r * u[1] + s * v[1], r * u[2] + s * v[2], r * u[3] + s * v[3]);
    }
    const uType = u?.type ?? typeof u;
    const vType = v?.type ?? typeof v;
...
export function inverse(m) {
    if (isMatrix(m)) {
        if (m.type === 'mat4') {
            ...
            return mat4(r0[0], r1[0], r2[0], r3[0], r0[1], r1[1], r2[1], r3[1], r0[2], r1[2], r2[2], r3[2], -dot(b, t), dot(a, t), -dot(d, s), dot(c, s));
        }
        else if (m.type === 'mat3') {
            ...
            return mat3(r0[0] * invDet, r1[0] * invDet, r2[0] * invDet, r0[1] * invDet, r1[1] * invDet, r2[1] * invDet, r0[2] * invDet, r1[2] * invDet, r2[2] * invDet);
        }
        else if (m.type === 'mat2') {
            const invDet = 1.0 / det(m);
            const invNeg = -invDet;
            return mat2(invDet * m[1][1], invNeg * m[0][1], invNeg * m[1][0], invDet * m[0][0]);
        }
    }
    const mType = m?.type ?? typeof m;
