MelonEngine.ProgramInstance = class {
    ...
    createProgram(gl, attributes = {}, shaderComponents = {}){
        ...
        ${shaderComponents.lighting ? 
            ...
            out vec3 vLightDirs[10]; //TODO: add out vec3 vnormals` : ''}
        ...
        ${attributes.albedoTexture ?`uniform sampler2D uAlbedoTexture;`: ''}
        out vec4 outColor;

        void main() {
            outColor = vec4(0);
            ${attributes.colorRGBA ? "outColor += vColor;" : ''}
            ${attributes.UVs && shaderComponents.albedoTexture ? `vec3 outColor += texture(uAlbedoTexture, (vUV * (uTexScale == vec2(0.0, 0.0) ? vec2 (1.0) : uTexScale)) + uTexOffset).rgb;` : ''}
            ${attributes.normals ? 
            `   for (int i = 0; i < uLightCount; i++) {
                    float lighting = max(dot(vNormal, vLightDirs[i]), 0.0);
                    outColor += lighting * vec4(uLightColors[i], 1.0);}` : ''}}`;
